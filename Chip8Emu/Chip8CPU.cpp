...
bool Chip8CPU::loadApplication(string filename)
{
	...
	if (file.is_open())
	{
		...
		if ((4096 - 0x200) > lSize)
		{
			...
			printf("ROM is larger than memory buffer!");
...
bool Chip8CPU::exists_test(const std::string& name) {
...
void Chip8CPU::emulateCycle()
{
	...
	const short NNN = opcode & 0x0FFF;
	const short NN = opcode & 0x00FF;
	const short N = opcode & 0x000F;
	const short X = (opcode & 0x0F00) >> 8;
	const short Y = (opcode & 0x00F0) >> 4;
